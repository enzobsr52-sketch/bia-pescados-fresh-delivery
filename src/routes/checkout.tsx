...
function Checkout() {
  ...
  const discount = mode === "varejo" ? subtotal * 0.05 : 0;
  ...
  async function startPix(event: React.FormEvent) {
    ...
    try {
      ...
      setError(message.includes("entregas somente") ? deliveryBlockedMessage : message.includes("PAGBANK_NOT_CONFIGURED") || message.includes("APP_ORIGIN_NOT_CONFIGURED") ? "O pagamento ainda está em configuração. Tente novamente mais tarde." : message);
      setStatus("form");
    }
  }
